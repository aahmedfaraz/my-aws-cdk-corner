...
export class Step19StepFunctionsStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    //	Appsync : API
    //================================================================================
    const appsyncApi = new appsync.GraphqlApi(this, `${service}-api`, {
      name: `${service}-api`,
    ...
    const step01_validate_student = new lambda.Function(
      this,
      `${service}-step01-validate-student-lambda`,
      {
    ...
    const step02_add_student = new lambda.Function(
      this,
      `${service}-step02-add-student-lambda`,
      {
    ...
    const step03_send_email = new lambda.Function(
      this,
      `${service}-step03-send-email-lambda`,
      {
    ...
    const step04_create_token = new lambda.Function(
      this,
      `${service}-step04-create-token-lambda`,
      {
    ...
    const step01_validate_student_sf = new stepfunctions_tasks.LambdaInvoke(
      this,
      `${service}-step01-validate-student`,
      {
        lambdaFunction: step01_validate_student,
      }
    );
    const step02_add_student_sf = new stepfunctions_tasks.LambdaInvoke(
      this,
      `${service}-step02-add-student`,
      {
        lambdaFunction: step02_add_student,
      }
    );
    const step03_send_email_sf = new stepfunctions_tasks.LambdaInvoke(
      this,
      `${service}-step03-send-email`,
      {
        lambdaFunction: step03_send_email,
      }
    );
    const step04_create_token_sf = new stepfunctions_tasks.LambdaInvoke(
      this,
      `${service}-step04-create-token`,
      {
        lambdaFunction: step04_create_token,
        outputPath: "$.Payload",
      }
    );
    const error_sf = new stepfunctions_tasks.LambdaInvoke(
      this,
      `${service}-error`,
      {
    ...
    // Choices
    // Check - 1
    const check_validation_choice = new stepfunctions.Choice(
      this,
      `${service}-check-validation-choice`
    );
    // Check - 2
    const check_db_status_choice = new stepfunctions.Choice(
      this,
      `${service}-check-db-status-choice`
    );
    // Check - 3
    const check_email_status_choice = new stepfunctions.Choice(
      this,
      `${service}-check-email-status-choice`
    );

    // Definition / Chain
    const definition = step01_validate_student_sf // step - 01
      .next(
        check_validation_choice
          .when(stepfunctions.Condition.isPresent("$.Payload.error"), error_sf) // error
          .when(
            stepfunctions.Condition.isNotPresent("$.Payload.error"),
            step02_add_student_sf // step - 02
              .next(
                check_db_status_choice
                  .when(
                    stepfunctions.Condition.isPresent("$.Payload.error"),
                    error_sf
                  ) // error
                  .when(
                    stepfunctions.Condition.isNotPresent("$.Payload.error"),
                    step03_send_email_sf // step - 03
                      .next(
                        check_email_status_choice
                          ...
                          .when(
                            stepfunctions.Condition.isNotPresent(
                              "$.Payload.error"
                            ),
    ...
    const httpDataSource = appsyncApi.addHttpDataSource(
      `${service}-appsync-http-datasource-1`,
      endpoint,
      {
